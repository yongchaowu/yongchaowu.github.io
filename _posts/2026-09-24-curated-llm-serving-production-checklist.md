...
> **技术交叉核对（2026-09-25）**：vLLM 的当前能力边界以 [官方文档](https://docs.vllm.ai/en/latest/) 为准；本文不把历史部署参数或吞吐数字当作当前版本保证。
...
节点内优先利用高速互联，跨节点优先考虑通信量和可观测性。不要只按 GPU 数量决定并行度，先确认显存容量、模型精度、KV Cache 预算和上下文上限。架构背景可回看 [多节点 LLM Serving 架构](#source-note-1)。
...
公式只能做早期估算，不能替代实测。尤其是 KV Cache、运行时工作区和框架版本都会改变显存边界。MiniCPM 的单机 Ollama/llama.cpp 记录可以作为轻量路径参考，但不应直接外推到 70B 或多节点服务：[MiniCPM5-1B 概览](#source-note-7)、[Ollama 部署](#source-note-8)、[llama.cpp 部署](#source-note-9)。
...
Ray、NCCL、共享存储和服务进程要分开验证。Ray 集群“节点在线”不代表 NCCL 通信正常；模型能加载也不代表长上下文和并发请求稳定。离线环境应使用 [Ray 离线安装记录](#source-note-4) 中的清单思路，并为每个依赖保留校验信息。

### 服务层

至少验证：健康检查、模型列表、普通请求、流式请求、超时、取消、错误响应和网关转发。多进程方案可参考 [New-API 双进程部署](#source-note-5)，但要确认健康检查、负载均衡和会话状态是否真的由该架构承担。
...
多节点部署的具体操作可结合 [vLLM + Ray Docker](#source-note-2) 与 [vLLM 集群实战](#source-note-3)，但要删掉未经评估的“关闭防火墙”“关闭交换分区”等通用化步骤，改成组织自己的安全基线。
