...
public: 
    ...
    {
        std::call_once(init_flag_, &ThreadPoolSingleton::init);
        return *instance_;
    }
    // 获取线程池对象
    BS::thread_pool<0>& get_thread_pool() { return thread_pool_; }
    // 可选：初始化时设置线程数（必须在首次使用前调用）
    static void initialize(unsigned int thread_count = std::thread::hardware_concurrency())
    {
        std::call_once(init_flag_, [thread_count]()
                       { instance_.reset(new ThreadPoolSingleton(thread_count)); });
    }

private:
    // 私有构造函数
    explicit ThreadPoolSingleton(unsigned int thread_count = std::thread::hardware_concurrency()) : thread_pool_(thread_count) {}
    // 删除拷贝构造函数和赋值运算符
    ThreadPoolSingleton(const ThreadPoolSingleton&) = delete;
    ThreadPoolSingleton& operator=(const ThreadPoolSingleton&) = delete;
    static std::unique_ptr<ThreadPoolSingleton> instance_;
    static std::once_flag init_flag_;
    BS::thread_pool<0> thread_pool_;
    // 初始化静态成员
...
int main()
{
    ThreadPoolSingleton::initialize(5);
    auto& pool = ThreadPoolSingleton::get_instance().get_thread_pool();

    std::cout<<"PoolCount:"<<pool.get_thread_count()<<std::endl;
    pool.submit_task([]{ std::cout<<"TTT"<<std::endl;return 1;});
    pool.wait();
