...
3. 给定一个链表和一个特定值x，对链表进行分隔，使得所有小于x的结点都在大于或等于x的结点之前。应当保留两个分区中 每个结点的初始相对位置。
...
解析：
- 用快慢指针。快指针先走k步，然后两个指针同时移动，当快指针到末尾时，慢指针的下一个位置是新的顺序的头结点。   当原链表为空时，直接返回NULL。当k大于链表长度和k远远大于链表长度时：首先需要遍历一遍原链表 得到链表长度n，然后k对n取余。
- 一个指针。原理：先遍历整个链表获得链表长度n，然后把链表头和尾链接起来，再往后走 n-k%n 个结点就到达新链表的头结点前一个点，这时断开链表即可。
...
深度优先方法：设置一个全局的数组visited[]用于存储顶点的访问情况，初始化全是0，并且用0表示没有访问过，用1表示访问过。当采用深度优先遍历方式时，若从i出发，遍历结束时，如果visited[j]=1,表示结点i和结点j之间有路径；否则没有路径。
...
深度优先方法：设置一个全局的数组visited[]用于存储顶点的访问情况，初始化全是0，并且用0表示没有访问过，用1表示访问过。当采用深度优先遍历方式时，若从i出发，遍历结束时，只存在结点j，如果visited[j]=0，表示不是连通图。
...
int maxDist(Graph G, int U){
	ArcNode *p;
	int queue[MaxVertex], front = 0, rear = 0, j, k, V;
	rear++;
	queue[rear]=U;
	visited[U] = 1;
	while(rear != front){
...
5. 已知某图的邻接矩阵为A，若从顶点i到顶点j有边，则A[i,j]=1;否则A[i,j]=0。试编写一算法求矩阵A的传递闭包C，使得若从顶点i到顶点j有一条或多条路径，则C[i,j]=1;否则C[i,j]=0。
typedef int adjmatrix[maxvtxnum][maxvtxnum];
void Change(adjmatrix A, adjmatrix C, int n);

分析：
首先，若从顶点i与顶点j存在直接路径，则顶点i，j联通；若顶点i存在到顶点k的路径，而顶点k存在到顶点j的路径，则顶点i，j同样联通，表达式为 C[i][j] = C[i][j] or (C[i][k] and C[k][j]) //C为传递闭包
用3层循环遍历即可求出传递闭包C，描述如下：
for 间接通过每个结点k
	for 每个结点i
		for 每个结点j
...
int DeleteHT(HT ht, int n, keytype K){
	head p,pre,tmp;
	if(ht[K % n]){  //该条链表不为空
		for(p=ht[K%n]; p!=NULL; pre=p, p=p->next){
			if(p->data.key == K){
				tmp = p;
				pre->next = p->next;
				free(tmp);
...
node *Search_Insert(bitptr root, int e){
	...
	if(p == NULL){
		...
		if((f->data).key > e) f->lchild = newnode;
		else f->rchild = newnode;
