...
[Git](#source-note-1) 的基本价值是建立可审计的变更轨迹。小团队可以从下面的最小约定开始：
...
[GitLab CI/CD](#source-note-3) 适合把检查分成三个阶段：
...
[Docker 入门记录](#source-note-9) 和 [以 ubuntu:latest 为例](#source-note-11) 提醒我们：标签不是版本。生产镜像应尽量固定基础镜像和依赖，数据、密钥、端口和日志目录通过运行时配置注入。
...
[GitLab 502](#source-note-4) 只是一个表象，可能来自反向代理、端口、容器、服务进程或资源耗尽。排查顺序应是：入口是否可达 → 反代后端是否健康 → 应用是否监听正确地址 → 日志是否有启动/权限错误 → 资源是否耗尽。

对于 [重置 root 账户](#source-note-6)、[清理 CI/CD jobs](#source-note-7) 和 [禁止开机自启动](#source-note-8)，先导出配置和备份，再执行破坏性操作，并记录执行人、时间和回滚命令。

## 5. 备份恢复：恢复演练比备份文件更重要

[GitLab 备份、恢复与迁移](#source-note-5) 的核心不是“压缩成功”，而是在干净环境中恢复成功。每次备份应记录：
